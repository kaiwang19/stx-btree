...
class Alex {
 ...
 public:
  ...
  struct Params {
    ...
    // insert latency
    int max_node_size = 1 << 24; // origin
    // int max_node_size = 1 << 6; // first try
  ...
  struct DerivedParams {
    ...
    // Assume the max node size of 64B
    // int max_fanout = (1 << 6) / 8;  // assumes 8-byte pointers
    // int max_data_node_slots = (1 << 6) / sizeof(V);
  };
...
// node's parent.
 ...
 private:
  ...
  void print_stat()
  {
    std::cout << "Num keys: " << stats_.num_keys
        << " Num Inner Nodes: " << stats_.num_model_nodes
        << " Num Leaf Nodes: " << stats_.num_data_nodes
        << "\n istats_: key_domain_["<< istats_.key_domain_min_ << ","<<istats_.key_domain_max_ << "]"
        << std::endl;  
  }
  
  public: void print_root_num_children() {
    if(!root_node_->is_leaf_)
    {
      std::cout << "num_children_ " << static_cast<model_node_type*>(root_node_)->num_children_ 
             << std::endl;  
 ...
 public:
  ...
  std::vector<V> query_range(const T& min_key, const T& max_key) const {
    std::vector<V> v_res;
    // std::cout << "range query(ALEX): " << min_key << " - " << max_key << std::endl;
    get_entries_recursion(root_node_, min_key, max_key, v_res);
    // std::cout << "v_res.size (ALEX) " << v_res.size() << std::endl;
  ...
  void get_entries_recursion(AlexNode<T, P>* cur_alexnode,
  const T& min_key, const T& max_key, std::vector<V>& v_res) const {
    if(!cur_alexnode->is_leaf_)
    {
      auto modelnode = static_cast<model_node_type*>(cur_alexnode);
      // if(print_detail) modelnode->print();

      int start_bucketID = modelnode->model_.predict(min_key);
      start_bucketID = std::min<int>(std::max<int>(start_bucketID, 0), modelnode->num_children_ - 1);
      int end_bucketID = modelnode->model_.predict(max_key);
      end_bucketID = std::min<int>(std::max<int>(end_bucketID, 0), modelnode->num_children_ - 1);

      // std::cout << " range query in MN "<< start_bucketID << ", " << end_bucketID << std::endl;
      int bucketID = start_bucketID;
      while(bucketID <= end_bucketID)
      {
        AlexNode<T, P>* cur_child = modelnode->children_[bucketID];
        int cur_child_repeats = 1 << cur_child->duplication_factor_;
        get_entries_recursion(cur_child, min_key, max_key, v_res);
        bucketID += cur_child_repeats;
      }
    }
    else
    {
      auto datanode = static_cast<data_node_type*>(cur_alexnode);
      
      int start_pos = datanode->find_key(min_key);
      if(start_pos < 0) {start_pos = datanode->find_lower(min_key);}
      int end_pos = datanode->find_key(max_key);
      if(end_pos < 0) {end_pos = datanode->find_upper(max_key)-1;}

      // std::cout << " range query in DN " << start_pos << ", " << end_pos << std::endl;
      int pos = start_pos;
      while(pos <= end_pos && pos <= datanode->last_pos())
      {
        v_res.push_back(datanode->get_entry(pos));
        pos = datanode->get_next_filled_position(pos, true);
      }    
    }
  }
